...
def main(ktu):
    ...
    logger.info('topic_names = ' + topic_names)
    ...
    if not topic_names:
        ...
        topic_list = topic_names.split(',')

    logger.info("topic list: {0}\n".format(topic_list))

    if not topic_list:
        sys.exit("No topics to validate.")
...
if __name__ == '__main__':
    ktu = KafkaTestUtils(logger, "kafkavalidatetopic{0}.log".format(int(time.time())), debug)
    main(ktu)
